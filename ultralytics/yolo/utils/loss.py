...
class WingLoss(nn.Module):
    def __init__(self, w=10, e=2):#原参数w =10, e=2
    ...
    def forward(self, x, t, sigma=1):
        ...
        y = flag * self.w * torch.log(1 + abs_diff / self.e) + (1 - flag) * (abs_diff - self.C)#用这种方式对函数进行分段
        return y.sum()

class LandmarksLoss(nn.Module):
    # BCEwithLogitLoss() with reduced missing label effects.
    """使用Wingloss代替欧氏距离，使用贝塞尔曲线采样"""
...
class BezierSampler(nn.Module):
    ...
    def __init__(self, order, num_sample_points, proj_coefficient=0):
        ...
        self.num_control_points = order + 1 #控制点比阶数大1
        self.num_sample_points = num_sample_points #采样点数
        self.control_points = []
    ...
    def get_bernstein_matrix(self):
        ...
        return c_matrix
    #c_matrix_size:num_control_points*num_control_points

    def get_sample_points(self, control_points_matrix): #转格式
        ...
        return upcast(self.bernstein_matrix).matmul(upcast(control_points_matrix))#乘法



# def point_loss(inputs, targets, valid_points=None):
...
class KeypointLoss(nn.Module):
    ...
    def __init__(self, sigmas) -> None:
        ...
        self.sigmas = sigmas
        ############
        self.bezier_sampler = BezierSampler(num_sample_points=20, order=4)#采样20个点，拟合4次多项式
        #######
    def forward(self, pred_kpts, gt_kpts, kpt_mask, area):
        """Calculates keypoint loss factor and Euclidean distance loss for predicted and actual keypoints. kpt_mask whether keypoint exist.  """
        ############wingloss对六个关键点的loss，未加入总的loss2023/7/24 为数值，已归一化
        landmarks_loss = LandmarksLoss(1.0)#修改点
        wing_mask = kpt_mask.reshape(-1, 5, 1).repeat(1, 1, 2)
        wing_loss = landmarks_loss(pred_kpts[..., [0,1]], gt_kpts[..., [0,1]], wing_mask)
        ############贝塞尔曲线,不需要逐个遍历，自带遍历，为数值，已归一化
        loss_curve = self.point_loss(self.bezier_sampler.get_sample_points(pred_kpts[..., [0,1]]), self.bezier_sampler.get_sample_points(gt_kpts[..., [0,1]]))
        loss_curve_mean = loss_curve.mean()
        ############# 掩码存在的意义就是将无效关键点置零
        ...
        return (0.1*wing_loss + loss_curve_mean)#wing_loss
        # return loss_curve_mean  # wing_loss
        #return kpt_loss_factor * loss_curve_mean #贝塞尔曲线loss
